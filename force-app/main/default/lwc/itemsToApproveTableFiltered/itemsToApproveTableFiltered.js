...
import checkCredentials from '@salesforce/apex/ReAuthController.checkCredentials';

export default class ItemsToApproveTableFiltered extends LightningElement {
    ...
    @api recordId;
    @api passwordLabel = "Enter Password";
    ...
    processApprovalAction(curRow) {
        if ((curRow || (this.selectedRows && this.selectedRows.length)) && this.currentAction) {
            checkCredentials({
                recordId: this.selectedRows[0].TargetObjectId,
                password: this.password,
                fieldName: 'Inventory__c',
                esigType: this.currentAction,
                comments: this.commentVal
            })
            .then(result => {
                var reAuthResponse = result;
                if (reAuthResponse.authPassed) {
                    ...
                    .then(result => {
                        ...
                        this.getServerData();
                        this.template.querySelector('c-uc-modal').closeModal();
                    })
                    .catch(error => {
                    ...
                    this.hasErrorMessage = true;
                    this.errorMessage = reAuthResponse.errorMessage;
                }
            })
            .catch(error => {
                this.hasErrorMessage = true;
